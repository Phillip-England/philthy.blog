...
func getRandomChristianQuestion() string {
	...
	questions := []string{
		...
		"What evidence is there that the Bible is more reliable than other ancient religious texts?",
	}
...
func Header(headerText string, subText string) string {
	return fmt.Sprintf(`
		<header class="flex flex-row justify-between p-4 border-b select-none z-40 bg-white fixed w-full top-0 h-[95px] dark:bg-black dark:text-white">
			<a href='/' class='flex dark:hidden'>
				<img src='/static/img/logo-black.svg' />
			</a>
...
func NavMenu(currentPath string) string {
	return fmt.Sprintf(`
		<nav id="navmenu" class="flex top-[95px] fixed bg-white w-2/3 h-full border-r z-40 hidden md:flex md:w-[300px] bg-white dark:bg-black text-black dark:text-white">
...
func NavItem(currentPath string, href string, text string) string {
	if currentPath == href {
		return fmt.Sprintf(`
		  <li class="rounded border w-full flex bg-gray-200 text-black">
		      <a class="w-full p-4" href="%s">%s</a>
		  </li>
		`, href, text)
	} else {
		return fmt.Sprintf(`
		  <li class="rounded border w-full flex bg-white dark:bg-black text-black dark:text-white">
		      <a class="w-full p-4" href="%s">%s</a>
		  </li>
		`, href, text)
	}
...
func SocialMediaBenner() string {
	return `
		<div class="text-black bg-white dark:bg-black dark:text-white border-b md:pl-[300px] flex flex-row fixed top-[95px] h-[65px] w-full z-30 justify-between">
			...
			<div>
				<div class='p-4 flex flex-row gap-4'>
					<svg _='on click toggle .dark on <html/> then set cookies.theme to "dark"' id='moon-icon' class="w-8 h-8 cursor-pointer flex dark:hidden"  aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
						<path fill-rule="evenodd" d="M11.675 2.015a.998.998 0 0 0-.403.011C6.09 2.4 2 6.722 2 12c0 5.523 4.477 10 10 10 4.356 0 8.058-2.784 9.43-6.667a1 1 0 0 0-1.02-1.33c-.08.006-.105.005-.127.005h-.001l-.028-.002A5.227 5.227 0 0 0 20 14a8 8 0 0 1-8-8c0-.952.121-1.752.404-2.558a.996.996 0 0 0 .096-.428V3a1 1 0 0 0-.825-.985Z" clip-rule="evenodd"/>
					</svg>
					<svg _='on click toggle .dark on <html/> then set cookies.theme to "light"' id='sun-icon' class="w-8 h-8 hidden cursor-pointer dark:flex" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
